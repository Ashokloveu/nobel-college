...
export default function AdminGalleryPage() {
  const [albums] = useState([
    { id: '1', title: 'Campus Infrastructure & Buildings', imageCount: 12, status: 'PUBLISHED' },
    { id: '2', title: 'Annual IT & Science Exhibition 2026', imageCount: 24, status: 'PUBLISHED' },
  ]);

  return (
    <AdminLayout
      title="Media & Gallery Album Management"
      subtitle="Organize photo albums, campus campus photography, and event coverage"
    >
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-bold text-white">Media Albums</h2>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-900/90 border-b border-slate-700/80 text-[11px] font-bold uppercase text-slate-400">
                <th className="py-4 px-4">Album Title</th>
                <th className="py-4 px-4">Image Count</th>
            ...
            <tbody className="divide-y divide-slate-700/60 text-xs">
              {albums.map((a) => (
                <tr key={a.id} className="hover:bg-slate-900/40">
                  <td className="py-4 px-4 font-bold text-white">{a.title}</td>
                  <td className="py-4 px-4 text-amber-400 font-bold">{a.imageCount} Photos</td>
